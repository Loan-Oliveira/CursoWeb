...
try:
  ...
  # Gera a planilha
  with pd.ExcelWriter("H:/Loan/Issues Resolvidos/13015 - Planilha beneficiarios DENTALUNI/Arquivos Gerados/Atualizacoes Automaticas/Atualização_Automática_" + data_formatada + ".xlsx", engine="openpyxl") as writer:
    try:
  ...
  try:
    with open('H:/Loan/Issues Resolvidos/13015 - Planilha beneficiarios DENTALUNI/Arquivos Gerados/Atualizacoes Automaticas/Atualizacao_Automatica_' + data_formatada + '.xlsx', 'rb') as arquivo_local:
      ftp.storbinary('STOR Atualização_Automática_' + data_formatada + '.xlsx', arquivo_local)
  except:
